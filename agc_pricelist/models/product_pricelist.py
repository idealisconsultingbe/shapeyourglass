...
class Pricelist(models.Model):
    ...
    def _compute_price_rule(self, products_qty_partner, date=False, uom_id=False):
        """ Overwritten method
            Use of sale order line cost as price if pricelist is based on line cost
        """
        self.ensure_one()
        if not date:
            ...
            for rule in items:
                ...
                if rule.base == 'pricelist' and rule.base_pricelist_id:
                    ...
                    price = rule.base_pricelist_id.currency_id._convert(price_tmp, self.currency_id, self.env.company,
                                                                        date, round=False)
                # Changes here
                # add condition to use line cost as price
                elif rule.base == 'line_cost':
                    price = self.env.context.get('line_cost', 0.0)
                # end of changes
                else:
